...
class SchedulerConfig:
    ...
    min_tokens_for_prefill: int = 2000
    """Minimum number of tokens to try to schedule for chunked prefills
    in a single scheduler iteration. If the scheduler cannot reach this
    minimum (e.g., because only decodes and a small amount of prefill fit
    the current budget), the scheduler will fall back to the regular
    behavior so that decoding can proceed."""
    
    min_step_tokens: int = 2000
    """(Offline) The preferred minimum number of tokens to schedule per
    scheduler step. When > 0 in an offline/throughput-oriented configuration,
    scheduler will try to reach this number of tokens per step by scheduling
    prefill requests after decode scheduling (without preempting decodes).
    This is a soft target constrained by `max_num_batched_tokens` and
    prefill concurrency (`max_num_seqs`). Default 0 (disabled).
    """
